...
while flag:
	...
	try:
		...
		pathList = []
		count = 0
		for path in nx.astar_path(G, source=int(switch[h2].split(":",1)[1]), target=int(switch[h1].split(":",1)[1]), heuristic=h, weight=None):
			if count == 0:
				pathList = [path]
			else:
		...
		# Cost Computation
		tmp = ""
		#for currentPath in nx.astar_path(G, source=int(switch[h2].split(":",1)[1]), target=int(switch[h1].split(":",1)[1]), heuristic=h, weight=None):
		for node in range(0,len(pathList)-1):
			tmp = tmp + str(pathList[node]) + "::"
			key = str(pathList[node])+ "::" + str(pathList[node+1])
			port = linkPorts[key]
			port = port.split(":",1)[0]
			port = int(port)
			stats = "http://localhost:8181/restconf/operational/opendaylight-inventory:nodes/node/openflow:"+str(pathList[node])+"/node-connector/openflow:"+str(pathList[node])+":"+str(port)
			getResponse(stats,"statistics")
		tmp = tmp + str(pathList[len(pathList)-1])
		tmp = tmp.strip("::")
